CREATE OR REPLACE FUNCTION public.preempt_search_service2(term character varying)
 RETURNS TABLE(pserviceid integer, s_serviceid integer, s_title character varying, s_description text, s_basecost numeric, s_averagepersonnel integer, s_averageruntime integer, searchrank real)
 LANGUAGE plpgsql
AS $function$ 
begin
	return query SELECT
